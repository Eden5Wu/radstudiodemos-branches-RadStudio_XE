...
function CreateConnectionIni(const AIniSection: string;
  AIniFile: string = ''): TSQLConnection; overload;
...
function CreateConnectionProps(
  ...
  const AExtra: string = ''): TSQLConnection; overload;
...
begin
  ...
  begin
    lNewParam := ADataSet.Params.AddParameter;
  ...
  begin
    ...
    try
      ...
      try
        ...
        driver := Ini.ReadString(AIniSection, cnDriver, SDefaultDriver);
        Result := CreateConnectionProps(database, username, password, driver,
          extra);
      finally
        lExtra.Free;
      end;
...
begin
  ...
  Result.Params.Text := Format(SCxnParams,
